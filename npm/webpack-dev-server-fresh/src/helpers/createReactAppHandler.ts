...
import type { PackageJson } from './sourceRelativeWebpackModules'
 ...
 */
export function createReactAppHandler ({ devServerConfig, sourceWebpackModulesResult }: PresetHandler) {
  if (!sourceWebpackModulesResult.framework) {
    throw new Error('Cannot source react-scripts without a framework')
  }
  ...
  const webpackConfig = loadWebpackConfig(sourceWebpackModulesResult.framework.importPath)
  ...
  if (isReactScripts5(sourceWebpackModulesResult.framework.packageJson)) {
...
function loadWebpackConfig (reactScriptsPath: string): Configuration {
  ...
  try {
    ...
    debug('webpack options: %o', webpackConfig)
...
function isReactScripts5 (json: PackageJson) {
  return Number(json.version[0]) >= 5
}
